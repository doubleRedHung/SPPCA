...
for i = (1:no_patches*2-1)
    for j = (1:no_patches*2-1)
        r1 = 1+(i-1)*ws/hh:(i+1)*ws/hh; % size: 1x16
        r2 = 1+(j-1)*ws/hh:(j+1)*ws/hh; % size: 1x16
...
% [mu, Sigma, wt] = Tyler_Sigma(X, 1);  
[mu, Sigma, wt, AR] = shape_V(X,scale_a*size(X,2), max_iter, 1); % scale is of order bigOh(dimensionality)
AR
...
wt =wt/sum(wt)*size(X,1); 
L2 = (diag(wt)*(X-repmat(mu',[size(X,1),1]))*Gamma)*Gamma'+repmat(mu',[size(X,1),1]);
...
for i = (1:no_patches*2-1)
    for j = (1:no_patches*2-1)
        ...
        r1 = 1+(i-1)*ws/hh:(i+1)*ws/hh;
        r2 = 1+(j-1)*ws/hh:(j+1)*ws/hh;
