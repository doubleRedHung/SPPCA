...
for i = (1:no_patches*2-1)
    for j = (1:no_patches*2-1)
        r1 = 1+(i-1)*ws/2:(i+1)*ws/2; % size: 1x16
...
scale_a = 1; 
max_iter =200;
[mu, Sigma,wt, AR] = shape_V(X,scale_a*size(X,2), max_iter); % scale is of order bigOh(dimensionality)
wt =wt/sum(wt); 
k=28; % k = rank(L) from robust PCA
[Gamma,eigVal]=svds(Sigma,k-1,"largest");
L2 = (diag(wt*size(X,1)).^.5*(X-repmat(mu',[size(X,1),1]))*Gamma) *Gamma'+repmat(mu',[size(X,1),1]);
